...
def main(conf: HydraConfig) -> None:
    ...
    # Set up mid-trajectory early stopping (off by default; configure via
    # early_stop.enabled=True in base.yaml or on the command line).
    ...
    for i_des in range(design_startnum, design_startnum + sampler.inf_conf.num_designs):
        if conf.inference.deterministic:
            make_deterministic(i_des + conf.inference.seed)
        ...
        if not sampler.contig_map.specific_mappings:
            writepdb(out, denoised_xyz_stack[0,:,:4], final_seq, sampler.binderlen, chain_idx=sampler.chain_idx, bfacts=bfacts)
        else:
            writepdb(out, denoised_xyz_stack[0,:,:4], final_seq, idx_pdb=sampler.out_idx, chain_idx=sampler.chain_idx, bfacts=bfacts)
        # run metadata
        trb = dict(
        ...
        if sampler.inf_conf.write_trajectory:
            ...
            if not sampler.contig_map.specific_mappings:
                ...
                out = f'{traj_prefix}_Xt-1_traj.pdb'
                writepdb_multi(out, denoised_xyz_stack, bfacts,
                    final_seq.squeeze(), use_hydrogens=False, backbone_only=False, chain_ids=sampler.chain_idx, idx_pdb=sampler.out_idx)
                out=f'{traj_prefix}_pX0_traj.pdb'
                writepdb_multi(out, px0_xyz_stack, bfacts,
                    final_seq.squeeze(), use_hydrogens=False, backbone_only=False, chain_ids=sampler.chain_idx, idx_pdb=sampler.out_idx)
